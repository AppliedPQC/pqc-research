...
> [`AppliedPQC/shrincs-rs`](https://github.com/AppliedPQC/shrincs-rs). The vocabulary is that of FIPS 205, and the section
...
Its known-answer tests were generated by running the reference Python extracted from the
draft, so they check the implementation against the document rather than against itself,
and they cover key generation, every stateful signature of two tree shapes, and the
stateless fallback, byte for byte. A separate check re-derives all sixteen sizes the
draft states from their defining equations, which is what would catch a mistyped
constant.
